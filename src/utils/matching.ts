...
export function calculateMatchScoreDetailed(params: MatchParams): MatchBreakdown {
  const n = (s: string) => s.toLowerCase().trim();
  ...
  const total = Math.min(Math.round(cityStateScore + proximityScore + routeOverlapScore +
    capacityScore + priceScore + dateScore + ratingBonus), 100);

  return { total, cityStateScore: Math.round(cityStateScore), proximityScore: Math.round(proximityScore), routeOverlapScore: Math.round(routeOverlapScore), capacityScore, priceScore, dateScore, ratingBonus, isBackhaul: false };
}

export function calculateBackhaulScore(params: MatchParams): number {
  const sd = params.shipmentDestCity.toLowerCase().trim();
  const so = params.shipmentOriginCity.toLowerCase().trim();
  const to = params.tripOriginCity.toLowerCase().trim();
  const td = params.tripDestCity.toLowerCase().trim();

  let score = 0;

  const originMatchesDest = sd === to || sd.includes(to) || to.includes(sd);
  const destMatchesOrigin = so === td || so.includes(td) || td.includes(so);

  if (originMatchesDest) score += 25;
  if (destMatchesOrigin) score += 25;

  if (originMatchesDest && destMatchesOrigin) {
    score += 10;
  }
